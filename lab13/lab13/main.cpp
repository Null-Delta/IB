...
unsigned int kBlockSize = 64;
unsigned int kBlockBitSize = (kBlockSize * 8);
 
unsigned int kOutputSize_512 = 64;
unsigned int kOutputSize_256 = 32;
...
void Mod512(byte* dst, byte* src, byte* add) {
    byte overrun = 0;
    
    for (byte i = kBlockSize; i-- > 0;) {
        overrun = src[i] + add[i] + (overrun >> 8);
        dst[i] = (byte)overrun;
    }
}

void Mod512(byte* dst, byte* src, uint32_t add) {
    for (byte i = kBlockSize; i-- > 0;) {
...
void XOR(byte* dst, byte* a, byte* b) {
    for (byte i = 0; i < kBlockSize; i++) dst[i] = a[i] ^ b[i];
}

class StribogGOST {
public:
    byte* stri_h = new byte[kBlockSize];
    byte* stri_N = new byte[kBlockSize];
    byte* stri_S = new byte[kBlockSize];
 
    int algorithm_size;
    
    StribogGOST(int size) {
        memset(stri_N, 0x00, kBlockSize);
        memset(stri_S, 0x00, kBlockSize);
 
        algorithm_size = size;
 
        if (algorithm_size == 512)
            memset(stri_h, 0, kBlockSize);
        else
            memset(stri_h, 1, kBlockSize);
    }
 
    void S(byte* state) {
        for (byte i = 0; i < kBlockSize; i++) state[i] = sbox[state[i]];
    }
 
    void PL(byte* state) {
        byte tmp[kBlockSize];
 
        memcpy(tmp, state, kBlockSize);
    ...
    void X(byte* dst, byte* a, byte* b) { XOR(dst, a, b); }
 
    void E(byte* dst, byte* k, byte* m) {
        byte K[kBlockSize];
    ...
    void G(byte* h, byte* N, byte* m) {
        byte hash[kBlockSize];
...
string Hashing(const string msg, int size) {
    StribogGOST gost(size);
 
    const unsigned char* message = reinterpret_cast<const unsigned char*>(msg.data());
    unsigned int len = (unsigned int)msg.size();
    byte m[kBlockSize];
  
    byte padding = kBlockSize - len;
    ...
    while (len >= kBlockSize) {
        ...
        gost.G(gost.stri_h, gost.stri_N, m);
  
        Mod512(gost.stri_N, gost.stri_N, kBlockBitSize);
        Mod512(gost.stri_S, gost.stri_S, m);
        
        len -= kBlockSize;
    }
 
    string result;
    for (int i = 0; i < (size == 512 ? kOutputSize_512 : kOutputSize_256); i++)
        result += gost.stri_h[i];
...
//        cout << endl;
//
//        cout << "256 bit hash: " << endl;
//        for (auto i : Hashing(text, 256))
//            cout << hex << (((uint32_t)i) & 0xff);
//        cout << endl << endl;
//
//        cout << "512 bit hash: " << endl;
//        for (auto i : Hashing(text, 512))
//            cout << hex << (((uint32_t)i) & 0xff);
//        cout << endl << endl;
//    }
}
